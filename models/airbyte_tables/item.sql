...
select
    md5(cast(coalesce(cast("name" as text), '') || '-' 
    || coalesce(cast(uoms as text), '') || '-' 
    || coalesce(cast(uoms_name as text), '') || '-' 
    || coalesce(cast(uoms_item_modified_by as text), '') || '-' 
    || coalesce(cast(uuid as text), '') || '-' 
    || coalesce(cast(brand as text), '') || '-' 
    || coalesce(cast(image as text), '') || '-' 
    || coalesce(cast("owner" as text), '') || '-' 
    || coalesce(cast(taxes as text), '') || '-' 
    || coalesce(cast(doctype as text), '') || '-' 
    || coalesce(cast(barcodes as text), '') || '-' 
    || coalesce(cast(creation as text), '') || '-' 
    || coalesce(cast(disabled as text), '') || '-' 
    || coalesce(cast(issynced as text), '') || '-' 
    || coalesce(cast(modified as text), '') || '-' 
    || coalesce(cast(docstatus as text), '') || '-' 
    || coalesce(cast(item_code as text), '') || '-' 
    || coalesce(cast(item_name as text), '') || '-' 
    || coalesce(cast(stock_uom as text), '') || '-' 
    || coalesce(cast(thumbnail as text), '') || '-' 
    || coalesce(cast(weightage as text), '') || '-' 
    || coalesce(cast("attributes" as text), '') || '-' 
    || coalesce(cast(item_group as text), '') || '-' 
    || coalesce(cast(description as text), '') || '-' 
    || coalesce(cast(end_of_life as text), '') || '-' 
    || coalesce(cast(modified_by as text), '') || '-' 
    || coalesce(cast(bundle_items as text), '') || '-' 
    || coalesce(cast(has_batch_no as text), '') || '-' 
    || coalesce(cast(has_variants as text), '') || '-' 
    || coalesce(cast(max_discount as text), '') || '-' 
    || coalesce(cast(minimumprice as text), '') || '-' 
    || coalesce(cast(no_of_months as text), '') || '-' 
    || coalesce(cast(safety_stock as text), '') || '-' 
    || coalesce(cast(customer_code as text), '') || '-' 
    || coalesce(cast(has_serial_no as text), '') || '-' 
    || coalesce(cast(is_sales_item as text), '') || '-' 
    || coalesce(cast(is_stock_item as text), '') || '-' 
    || coalesce(cast(item_defaults as text), '') || '-' 
    || coalesce(cast(min_order_qty as text), '') || '-' 
    || coalesce(cast(naming_series as text), '') || '-' 
    || coalesce(cast(opening_stock as text), '') || '-' 
    || coalesce(cast(retain_sample as text), '') || '-' 
    || coalesce(cast(website_image as text), '') || '-' 
    || coalesce(cast(asset_category as text), '') || '-' 
    || coalesce(cast(customer_items as text), '') || '-' 
    || coalesce(cast(is_fixed_asset as text), '') || '-' 
    || coalesce(cast(lead_time_days as text), '') || '-' 
    || coalesce(cast(publish_in_hub as text), '') || '-' 
    || coalesce(cast(reorder_levels as text), '') || '-' 
    || coalesce(cast(supplier_items as text), '') || '-' 
    || coalesce(cast(has_expiry_date as text), '') || '-' 
    || coalesce(cast(sample_quantity as text), '') || '-' 
    || coalesce(cast(show_in_website as text), '') || '-' 
    || coalesce(cast(synced_with_hub as text), '') || '-' 
    || coalesce(cast(warranty_period as text), '') || '-' 
    || coalesce(cast(weight_per_unit as text), '') || '-' 
    || coalesce(cast(create_new_batch as text), '') || '-' 
    || coalesce(cast(is_item_from_hub as text), '') || '-' 
    || coalesce(cast(is_purchase_item as text), '') || '-' 
    || coalesce(cast(no_of_months_exp as text), '') || '-' 
    || coalesce(cast(valuation_method as text), '') || '-' 
    || coalesce(cast(variant_based_on as text), '') || '-' 
    || coalesce(cast(country_of_origin as text), '') || '-' 
    || coalesce(cast(has_excel_serials as text), '') || '-' 
    || coalesce(cast(auto_create_assets as text), '') || '-' 
    || coalesce(cast(shelf_life_in_days as text), '') || '-' 
    || coalesce(cast(asset_naming_series as text), '') || '-' 
    || coalesce(cast(saleswarrantymonths as text), '') || '-' 
    || coalesce(cast(total_projected_qty as text), '') || '-' 
    || coalesce(cast(website_item_groups as text), '') || '-' 
    || coalesce(cast(delivered_by_supplier as text), '') || '-' 
    || coalesce(cast(allow_alternative_item as text), '') || '-' 
    || coalesce(cast(is_sub_contracted_item as text), '') || '-' 
    || coalesce(cast(over_billing_allowance as text), '') || '-' 
    || coalesce(cast(purchasewarrantymonths as text), '') || '-' 
    || coalesce(cast(website_specifications as text), '') || '-' 
    || coalesce(cast(enable_deferred_expense as text), '') || '-' 
    || coalesce(cast(enable_deferred_revenue as text), '') || '-' 
    || coalesce(cast(show_variant_in_website as text), '') || '-' 
    || coalesce(cast(is_customer_provided_item as text), '') || '-' 
    || coalesce(cast(default_material_request_type as text), '') || '-' 
    || coalesce(cast(include_item_in_manufacturing as text), '') || '-' 
    || coalesce(cast(standard_rate_aibyte_transform as text), '') || '-' 
    || coalesce(cast(over_delivery_receipt_allowance as text), '') || '-' 
    || coalesce(cast(valuation_rate_aibyte_transform as text), '') || '-' 
    || coalesce(cast(inspection_required_before_delivery as text), '') || '-' 
    || coalesce(cast(inspection_required_before_purchase as text), '') || '-' 
    || coalesce(cast(last_purchase_rate_aibyte_transform as text), '') as text)) as _airbyte_item_hashid,
    tmp.*
